...
void setup_mapping(uint32_t index[], uint32_t mapping[], char *path)
{
    char buff[128];
    int pointIdx = 0;
    int charIdx = 0;
...
void update_mapping(uint32_t index[], uint32_t mapping[], uint32_t t_code_point, uint32_t s_code_point)
{
...
#define TRUE 1
#define FALSE 0
void parse_mapping_fix(uint32_t index[], uint32_t mapping[], char *path, uint8_t with_delimiter)
{
    char buff[1024];
    int pointIdx = 0;
    int charIdx = 0;
    ...
    while (fgets(buff, 1024, fp) != NULL)
    {
        ...
        update_mapping(index, mapping, t_code_point, s_code_point);
    }
    fclose(fp);
}

void save_to_file(uint32_t index[], uint32_t mapping[], char *path)
{
...
int main(int argc, char const *argv[])
{
    ...
    setup_mapping(index, mapping, "C:\\codespace\\naming\\ts.txt");
    parse_mapping_fix(index, mapping, "C:\\Users\\luhong\\Downloads\\ts.txt", TRUE);
    save_to_file(index, mapping, "C:\\codespace\\naming\\ts_new.txt");
    return 0;
}
